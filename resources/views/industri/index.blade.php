...
<div class="bg-blue-500 text-white p-4 flex items-center">
    ...
    <h1 class="text-lg font-semibold">Peta Industri</h1>
</div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // DATA LARAVEL
    let industri = @json($industri);
    console.log("DATA INDUSTRI:", industri);

    // ICON MARKER
    let iconIndustri = L.icon({
        ...
        if(d.latitude && d.longitude){
            ...
            marker.bindPopup(`
                <div style="font-size:13px;">
                    <b>Nama Industri: ${d.nama ?? '-'}</b><br>
                    Deskripsi: ${d.deskripsi ?? '-'}<br>
                    Jumlah: ${d.jumlah ?? '-'}<br>
                    ${d.foto ? `<img src="/img/industri/${d.foto}" width="200" style="margin-top:5px;border-radius:8px;">` : ''}
